...
//@property (nonatomic, readwrite) ASICachePolicy cachePolicy;
//@property (nonatomic, readwrite) ASICacheStoragePolicy cacheStoragePolicy;
...
- (BOOL)shouldTimeOut
{
	...
	if ([self readStream] && [self readStreamIsScheduled] && [self lastActivityTime] && [self timeOutSeconds] > 0 && secondsSinceLastActivity > [self timeOutSeconds]) {
        ...
        } else if (secondsSinceLastActivity > [self timeOutSeconds]*3.0){//*1.5) {
            NSLog(@"***** ASIHTTPRequest shouldTimeout returns yes");
			return YES;
		}
	}
	return NO;
